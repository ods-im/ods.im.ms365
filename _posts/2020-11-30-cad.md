...
* 8、将下载的激活文件解压出来，将acad.exe替换CAD安装目录下的acad.exe文件。替换之后即激活成功！（建议先备份安装目录下的acad文件）
